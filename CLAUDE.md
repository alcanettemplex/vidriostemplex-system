...
La ruta del repo cambia según la máquina (`C:\dev\vidrios-templex-system` en una, `C:\Users\User\Desktop\vidrios-templex-system` en otra). Lo que no cambia es que debe quedar fuera de cualquier carpeta que sincronice OneDrive: OneDrive sincronizando `.git/` corrompe el repo y resuelve conflictos renombrando archivos en vez de mezclarlos. Si la ruta de trabajo cae dentro de OneDrive, avisarle al usuario.

- **Al iniciar sesión:** si `.claude/settings.json` tiene el hook `SessionStart`, este corre `git fetch` e informa si la rama está atrás o adelante del remoto; si no lo tiene, hacer el `git fetch` y la comparación a mano. Si hay commits por traer, **decírselo al usuario y esperar su orden** — nunca hacer `pull` por iniciativa propia; un pull sobre un working tree sucio puede mezclar sin que él lo vea.
...
- El hook (`.claude/hooks/session-start.sh`) vive en `.claude/`, que está en `.gitignore` — **no viaja por git**. Hay una copia versionada en `tooling/claude-code/` con las instrucciones para instalarlo en la otra máquina.
...
**Módulos clave:**
- **Proveedores (ingesta FE):** las mismas reglas gobiernan la ingesta de facturas y la **importación de listas de precios** (Fase 3, 2026-09-03): fecha de vigencia sobre orden de carga, la modalidad decide qué precio se toca, y un código desconocido va a la bandeja en vez de adivinarse. La lista **previsualiza antes de escribir** (`dry_run`) y comparte el derivador `SD-<hash>` del parser, para que lista y factura del mismo ítem caigan en la misma fila. Reglas no negociables, todas verificadas el 2026-08-30 — (1) el histórico registra **cambios de precio, no apariciones**; (2) el precio vigente lo define la **fecha de la factura**, no el orden de carga: el lote se ordena por `fecha_emision` y una factura anterior se archiva con `retroactivo=true` sin desplazar el vigente; (3) la **modalidad** decide qué precio se actualiza — un `unitCode` informativo (`MTR`, `KGM`, `MTK`) exige coincidir con `unidad_compra`, uno genérico (`94`, `EA`, `NIU`) solo vale si hay una única equivalencia; (4) idempotencia por **CUFE completo** en `factura_proveedor_procesada`, nunca por substring de `documento_ref`; (5) notas crédito/débito y monedas ≠ COP se registran **sin mover precios**; (6) el mapeo siempre lo confirma un humano; (7) **`cbc:PriceAmount` se desambigua por reconciliación exacta contra el total de línea** — según UBL es el precio de `BaseQuantity` unidades, pero muchos emisores repiten ahí la cantidad facturada; gana la lectura que cumple `PriceAmount × cantidad − descuento + cargo == LineExtensionAmount` (se reconcilia contra el bruto de línea, `total + descuento − cargo`, para que también cuadren las líneas regaladas donde el total es cero).; (8) **el precio que se registra es el NETO de descuentos de línea** (2026-09-19) — NIC 2 §11, y porque el comparador ordena por `precio_actual ASC` y con brutos el ranking puede quedar invertido. El descuento de línea (`InvoiceLine/cac:AllowanceCharge`, reduce base IVA) se deduce; el de documento (pronto pago, no reduce base IVA) solo genera aviso; `Price/cac:AllowanceCharge` es descriptivo y **no** se resta. **Una línea al 100% de descuento es una bonificación y no actualiza ningún precio** — registrarla en cero borraría el costo y lo propagaría al Cotizador. La misma regla rige el mapeo manual y la ingesta automática: si difieren, `precio_actual` deja de ser comparable. Se guardan `precio_bruto`/`descuento_pct`/`descuento_valor`/`cantidad`/`total_linea`; `NULL` = fila anterior al cambio, en bruto y no recalculable (los XML no se persisten), que se corrige sola con la próxima factura. **(9) Un proveedor puede facturar el mismo producto con VARIOS códigos** (2026-09-21) — GRUPO ROLDAN usa `GRE701NG` y `GRP701NG` para el mismo sillar 7038. Los códigos viven en `ProveedorProductoCodigo` y todo lookup pasa por `resolverEquivalenciasPorCodigo()` de `utils/proveedorCodigos.ts`, nunca por `pp.codigo_proveedor` directo. Si dos códigos de la misma factura apuntan a la misma fila, se aplica el **precio mayor** y se emite el aviso `CODIGOS_ALIAS_MISMA_FACTURA` — decisión del usuario, coherente con el `maxPrecio` que ya rige dentro de un código. Ver `TECH_DEBT.md` 2026-08-30 y `docs/modulos/compras.md`.
- **SAP:** `SAP → SAPItem → OrdenCompra (ODC) → ODCItem`
- **PedidoPV:** auto-generado al crear ODP con `proveedor_vidrio`. Base 6733. **El tope de ítems por formulario lo fija el proveedor: 29 para Templacol, 12 para el resto** (`utils/pedidoPvCapacidad.ts`); al superarlo se crean extensiones `-1,-2...`. El **formulario** (Excel e impreso) también se elige por proveedor: Templacol usa el suyo, Vitelsa y cualquier otro valor usan el de Vitelsa. Cambiar `proveedor_vidrio` en la ODP **propaga** el cambio a todos sus Pedidos PV en cualquier estado, re-particiona si el formato nuevo admite menos ítems y emite `emitirCambio('pedidos_pv')`; vaciar el proveedor con un pedido ya creado se rechaza con 409. Crear el pedido y propagarle el proveedor son **un solo bloque** en `updateODP`, no dos condiciones independientes: separadas, una ODP con `proveedor_vidrio = ''` y pedido ya creado no cae en ninguna y el pedido conserva el proveedor viejo. La comparación es normalizada: `' Vitelsa '` y `'vitelsa'` no cuentan como cambio. Como red de seguridad, el **formato** (Excel e impreso) de un pedido con `origen='SISTEMA'` lo decide `proveedorParaFormato()` — manda `odp.proveedor_vidrio`, no la fila del pedido, porque el papel que sale al proveedor es irreversible; un pedido `MANUAL` conserva el suyo. La vía inversa (`PATCH /api/pedidos-pv/:id`, que acepta `proveedor`) sigue pudiendo desalinear la fila, aunque ya no el formato — ver `TECH_DEBT.md` 2026-09-03.
...
| `ProveedorProducto` | `proveedor_producto` | Equivalencia (proveedor, producto, **modalidad**). UNIQUE `(proveedor_id, catalogo_producto_id, unidad_compra)`. Precio vigente + 2 anteriores denormalizados. Baja lógica, nunca borrado: el `CASCADE` arrastraría el histórico. `codigo_proveedor` ya **no es la fuente de verdad** desde 2026-09-21: es la copia de lectura del código principal, que vive en `ProveedorProductoCodigo` |
| `ProveedorProductoCodigo` | `proveedor_producto_codigo` | Los **N códigos** con los que un proveedor factura el mismo producto (2026-09-21). UNIQUE `(proveedor_producto_id, codigo_proveedor)` — **no** `(proveedor_id, codigo_proveedor)`: el proveedor 1029 usa `3`, `11` y `32` en dos modalidades del mismo producto. Que un código apunte a dos **productos** distintos se rechaza con 409 desde la aplicación. `proveedor_id` denormalizado para el lookup por índice de la ingesta |
| `ProveedorProductoPrecio` | `proveedor_producto_precio` | Histórico. **Solo registra cambios de precio, no apariciones.** `retroactivo=true` cuando la factura es anterior a la vigente. `precio` es el **neto**; `precio_bruto`/`descuento_pct`/`descuento_valor`/`cantidad`/`total_linea` (2026-09-19) vuelven la fila **recalculable** — `NULL` = cargada antes de ese cambio |
| `ProveedorCodigoPendiente` | `proveedor_codigo_pendiente` | Bandeja sin mapear. UNIQUE `(proveedor_id, codigo_proveedor)`. `PENDIENTE\|MAPEADO\|DESCARTADO`. `precio_detectado` es el **neto**; los `*_detectado` del desglose alimentan la cadena lista→descuento→neto del modal de vinculación |
...
**Sistema visual "Cristal y Aluminio" (2026-09-25)** — detalle y reglas en `design/sistema-visual/README.md`:
- La legibilidad se controla en `tailwind.config.js`, no clase por clase: `slate` y `gray` son una escala reajustada ("aluminio", tonos medios más oscuros), `templex-*` es el azul de marca, `font-black` = 800. Solo se tocaron los **neutros**; los colores con significado de estado no.
- Fuente Geist (`@fontsource-variable`, sin CDN) con cifras tabulares. `font-sans`/`font-mono` apuntan a las variables `--font-sans`/`--font-mono` de `index.css`; así la ventana de impresión puede cambiarlas sin tocar los imprimibles.
- **Iconos: importar siempre de `components/ui/icons.ts`**, nunca de `@phosphor-icons/react` ni de `lucide-react` (desinstalado). El registro conserva los nombres que el ERP usaba con lucide. Peso por defecto `bold` (vía `IconContext` en `app/App.tsx`); `duotone` para iconos protagonistas.
- Menú y barra superior leen el mapa de módulos de `components/common/navegacion.ts`: un módulo nuevo del menú se agrega ahí.
...
**No escribir `cd <ruta> && <comando>`.** La herramienta lo evalúa como comando compuesto y **pide permiso aunque el comando esté en el allowlist**. No se arregla agregando entradas al allowlist; se arregla no usando `cd`.
...
**Los permisos dependen de la máquina.** La configuración de referencia está en `tooling/claude-code/` (su `README.md` explica cómo instalarla): sin `bypassPermissions`, con reglas `deny` en `.claude/settings.json` contra `rm -rf`, `git reset --hard`, `git clean` y `Remove-Item -Recurse` (Bash y PowerShell, también con `git -C`) —no preguntan, bloquean, y son el único candado técnico contra borrar el working tree con un comando mal formado— y el hook `SessionStart` descrito arriba. Ni `.claude/` (en `.gitignore`) ni `~/.claude/` viajan por git, así que cada máquina tiene solo lo que se le copió a mano: antes de contar con las reglas `deny`, leer `.claude/settings.json`, y si faltan, avisarle al usuario. El modo de permisos se fija al arrancar la sesión y se cambia con `shift+tab`.

Sea cual sea el modo, la metodología "propongo → preguntas → plan → **procede**" y la regla de Commits no dependen de los permisos: que `git push` no pida confirmación no autoriza a hacer commit ni push por iniciativa propia.
