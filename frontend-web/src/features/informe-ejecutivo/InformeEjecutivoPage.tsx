...
const Semaforo: React.FC<{ estado: string; label: string; desc?: string }> = ({ estado, label, desc }) => {
  const cfg: Record<string, { bg: string; icon: React.ReactNode }> = {
    verde:    { bg: 'bg-emerald-50 border-emerald-200 text-emerald-800', icon: <CheckCircle weight="duotone" className="w-[18px] h-[18px] text-emerald-600" /> },
    amarillo: { bg: 'bg-amber-50 border-amber-200 text-amber-800',       icon: <AlertTriangle weight="duotone" className="w-[18px] h-[18px] text-amber-600" /> },
    rojo:     { bg: 'bg-red-50 border-red-200 text-red-800',             icon: <XCircle weight="duotone" className="w-[18px] h-[18px] text-red-600" /> },
  };
  const c = cfg[estado] || cfg['amarillo'];
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-[13px] font-semibold ${c.bg}`}>
      {c.icon} {label}
...
const KPICard: React.FC<{
  ...
  // Acento de color arriba + fondo blanco: el color conserva su significado sin teñir la cifra.
  ...
  return (
    <div className="relative overflow-hidden bg-white border border-slate-200 rounded-2xl shadow-card p-4 pt-[18px] min-w-0">
      <span className={`absolute inset-x-0 top-0 h-[3px] ${colores[color] || colores.blue}`} />
      <div className="flex items-start gap-1.5 min-h-[36px]">
        <p className="text-[12px] font-semibold text-slate-900 uppercase tracking-wide leading-[18px]">{label}</p>
        {desc && <span className="mt-[3px]"><InfoTooltip text={desc} /></span>}
      </div>
      <p className="text-[22px] sm:text-[24px] leading-tight font-extrabold tracking-tight text-slate-900 tabular-nums whitespace-nowrap">{value}</p>
      {sub && <p className="text-[12px] text-slate-700 mt-1">{sub}</p>}
      {delta !== undefined && delta !== null && (
        <div className={`flex items-center gap-1 mt-1.5 text-[12px] font-semibold ${delta > 0 ? 'text-emerald-700' : delta < 0 ? 'text-red-700' : 'text-slate-700'}`}>
...
const NivelBadge: React.FC<{ nivel: string }> = ({ nivel }) => {
  ...
  return (
    <span className={`px-2 py-0.5 text-[11px] font-semibold border rounded-full ${cfg[nivel] || cfg.atencion}`}>
...
const AreaBadge: React.FC<{ area: string }> = ({ area }) => {
  ...
  return (
    <span className={`px-2 py-0.5 text-[11px] font-semibold rounded-md ${cfg[area] || 'bg-slate-100 text-slate-800'}`}>
      {labels[area] || area}
    </span>
  );
};
...
const Filtros: React.FC<FiltrosProps> = ({
  ...
  <div className="flex flex-wrap items-end gap-3 mb-5 p-3 bg-white rounded-2xl border border-slate-200 shadow-card">
    <div>
      <label className="text-[11px] font-semibold text-slate-900 uppercase tracking-wide block mb-1">Desde</label>
    ...
    <div>
      <label className="text-[11px] font-semibold text-slate-900 uppercase tracking-wide block mb-1">Hasta</label>
    ...
    <div className="flex gap-1">
      {[
        ...
        <button key={b.label} onClick={b.fn}
          className="px-2.5 py-1.5 text-xs font-medium text-slate-900 bg-white border border-slate-300 rounded-lg hover:bg-slate-100 transition">
          {b.label}
        </button>
      ))}
    </div>
    {asesores && onAsesorId && (
      <div>
        <label className="text-[11px] font-semibold text-slate-900 uppercase tracking-wide block mb-1">Asesor</label>
    ...
    <button onClick={onRefresh} disabled={loading}
      className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 text-white text-xs font-semibold rounded-lg hover:bg-slate-700 disabled:opacity-50 transition">
...
const TablaVacia: React.FC<{ msg?: string }> = ({ msg = 'Sin datos para mostrar' }) => (
  <div className="text-center py-8 text-slate-700 text-sm">{msg}</div>
);
        ...
        <>
          ...
          <div className="mb-5">
            <div className="flex items-center gap-2 mb-2">
              <p className="text-[13px] font-semibold text-slate-900">Salud del Sistema</p>
          ...
          {d.odps_por_dia?.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                <p className="text-[13px] font-semibold text-slate-900">ODPs Creadas por Día</p>
              ...
              <div className="flex items-end gap-1.5 h-32 bg-white rounded-2xl p-3 border border-slate-200 shadow-card">
                {d.odps_por_dia.map((row: { dia: string; total: number }) => {
                  ...
                  return (
                    <div key={row.dia} className="flex-1 h-full flex flex-col justify-end items-center gap-1" title={`${row.dia}: ${row.total}`}>
                      <span className="text-[11px] text-slate-700">{row.total}</span>
                      {/* Contenedor con altura propia: el % de la barra se mide contra el área útil, no contra la columna con rótulos */}
                      <div className="w-full flex-1 min-h-0 flex items-end">
                        <div className="w-full bg-blue-500 rounded-t-md" style={{ height: `${Math.max(pct, 4)}%` }} />
                      </div>
                      <span className="text-[11px] text-slate-700 truncate max-w-full">{row.dia.slice(5)}</span>
                    </div>
                  );
                })}
              </div>
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        <button onClick={() => setSubTab('todos')}
          className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition ${subTab === 'todos' ? 'bg-slate-800 text-white' : 'bg-white border border-slate-300 text-slate-800 hover:bg-slate-50'}`}>
          Todos
        </button>
        {lista.map(a => (
          <button key={a.asesor_id} onClick={() => setSubTab(String(a.asesor_id))}
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition ${subTab === String(a.asesor_id) ? 'bg-slate-800 text-white' : 'bg-white border border-slate-300 text-slate-800 hover:bg-slate-50'}`}>
            {a.nombre.split(' ')[0]}
          </button>
        ))}
      </div>
      ...
      {subTab === 'todos' ? (
        <div className="overflow-x-auto">
          ...
          {lista.length === 0 ? <TablaVacia /> : (
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-100 text-[11px] font-semibold text-slate-900 uppercase tracking-wide">
              ...
              <tbody>
                {lista.map((a, i) => {
                  ...
                  const pctColor = pct === null ? 'text-slate-700' : pct >= 100 ? 'text-emerald-600' : pct >= 70 ? 'text-amber-600' : 'text-red-600';
                  return (
                    <tr key={a.asesor_id} className={`border-b border-slate-100 hover:bg-slate-50 cursor-pointer ${subTab === String(a.asesor_id) ? 'bg-blue-50' : ''}`}
                      ...
                      <td className="px-3 py-2 text-slate-700 text-xs">#{i + 1}</td>
                      <td className="px-3 py-2 font-semibold text-slate-800">{a.nombre}</td>
                      <td className="px-3 py-2 text-right text-slate-800">{a.meta > 0 ? COP(a.meta) : '—'}</td>
                      <td className="px-3 py-2 text-right font-semibold text-slate-900 tabular-nums">{COP(a.real)}</td>
                      <td className={`px-3 py-2 text-right font-semibold tabular-nums ${pctColor}`}>
                        {pct !== null ? `${pct}%` : '—'}
                      </td>
                      ...
                      <td className="px-3 py-2 text-right font-semibold text-slate-900">{a.tasa_conversion}%</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div className="col-span-full">
            <h3 className="font-bold text-lg text-slate-900">{asesorDetalle.nombre}</h3>
            <p className="text-xs text-slate-700 uppercase">{asesorDetalle.rol}</p>
          </div>
          ...
          {asesorDetalle.meta > 0 && (
            <div className="col-span-full">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-[13px] font-semibold text-slate-900">Avance de Meta</p>
              ...
              <p className="text-xs text-slate-700 mt-0.5">{asesorDetalle.pct_meta || 0}% completado</p>
            </div>
          )}
        </div>
      ) : null}
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {secciones.map(s => (
          <button key={s.id} onClick={() => setSeccion(s.id as typeof seccion)}
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition ${seccion === s.id ? 'bg-slate-800 text-white' : 'bg-white border border-slate-300 text-slate-800 hover:bg-slate-50'}`}>
            {s.label}
          </button>
        ))}
      </div>
                ...
                <div key={o.id} className="bg-white border border-red-200 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <span className="ml-2 text-sm text-slate-800">{o.cliente}</span>
                      <span className="ml-2 text-xs text-slate-700">/ {o.asesor}</span>
                    </div>
                    <span className="text-xs font-semibold text-red-700 tabular-nums bg-red-50 border border-red-200 px-2 py-0.5 rounded-full whitespace-nowrap">
        ...
        <div>
          ...
          {d.listas_sin_programar?.length === 0 ? <TablaVacia msg="Todas las ODPs listas están programadas ✓" /> : (() => {
            ...
            const chips: { key: typeof listasFilter; label: string; color: string }[] = [
              { key: 'todos',       label: `Todas (${conteos.todos})`,              color: listasFilter === 'todos'       ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-800 hover:bg-slate-200' },
                    ...
                    <tbody>
                      {listaFiltrada.map((o: any) => (
                        <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                          <td className="px-3 py-2 font-semibold text-slate-900 whitespace-nowrap">{o.numero_odp}</td>
                          <td className="px-3 py-2 text-slate-700">{o.cliente}</td>
                          <td className="px-3 py-2 text-slate-700 text-xs">{o.asesor}</td>
                          <td className="px-3 py-2">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${badgeStyle(o)}`}>{badgeLabel(o)}</span>
                          </td>
                          <td className="px-3 py-2 text-right font-semibold text-amber-700">{o.dias_lista ?? '—'}d</td>
                          <td className="px-3 py-2 text-right text-slate-700">{o.fecha_entrega ? o.fecha_entrega.slice(0, 10) : '—'}</td>
                          <td className="px-3 py-2 text-right text-amber-700 font-semibold tabular-nums">{COP(o.pendiente)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <tbody>
                {d.atrasadas.map((o: any) => (
                  <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 text-slate-700">{o.cliente}</td>
                    <td className="px-3 py-2 text-slate-700 text-xs">{o.asesor}</td>
                    <td className="px-3 py-2 text-xs text-slate-800">{LABELS_ESTADO[o.estado_produccion] || o.estado_produccion}</td>
                    <td className="px-3 py-2 text-right font-semibold text-red-700">{o.dias_retraso}d</td>
                    <td className="px-3 py-2 text-right text-amber-700 font-semibold tabular-nums">{COP(o.pendiente)}</td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {d.pausadas.map((o: any) => (
                  <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 text-slate-700">{o.cliente}</td>
                    <td className="px-3 py-2 text-slate-700 text-xs">{o.asesor}</td>
                    <td className="px-3 py-2 text-right text-slate-700">{o.fecha_creacion ? o.fecha_creacion.slice(0, 10) : '—'}</td>
                    <td className="px-3 py-2 text-right text-amber-700 font-semibold tabular-nums">{COP(o.pendiente)}</td>
                  </tr>
                ))}
              </tbody>
            ...
            {d.embudo?.filter((e: any) => e.total > 0).map((e: any) => {
              ...
              return (
                <div key={e.estado} className="flex items-center gap-3">
                  <span className="text-xs font-semibold text-slate-800 w-40 shrink-0">{LABELS_ESTADO[e.estado] || e.estado}</span>
                  <div className="flex-1 h-5 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full ${barColor} rounded-full transition-all`} style={{ width: `${pct}%` }} />
                  </div>
                  <span className="text-sm font-semibold text-slate-900 tabular-nums w-6 text-right">{e.total}</span>
                </div>
              );
            })}
          </div>
        ...
        <>
          <div className="flex flex-col gap-3 mb-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 border border-emerald-100 bg-emerald-50/40 rounded-xl p-3">
          ...
          {d.meta_empresa > 0 && (
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-[13px] font-semibold text-slate-900">Meta Empresa del Período</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-700">de {COP(d.meta_empresa)}</span>
              </div>
            </div>
          )}
          {d.distribucion_forma_pago?.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                <p className="text-[13px] font-semibold text-slate-900">Distribución por Forma de Pago</p>
              ...
              <div className="flex flex-wrap gap-2">
                {d.distribucion_forma_pago.map((r: any) => (
                  <div key={r.forma_pago} className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm">
                    ...
                    <p className="text-xs text-slate-700">{r.total} ODP{Number(r.total) !== 1 ? 's' : ''} · {COP(Number(r.valor || 0))}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {asesoresUnicos.map(a => (
                    <button key={a} onClick={() => setMatpagoAsesor(a)}
                      className={`px-3 py-1 rounded-full text-xs font-semibold transition-colors cursor-pointer ${matpagoAsesor === a ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-800 hover:bg-slate-200'}`}>
                    ...
                    <tbody>
                      {listaFiltrada.map((o: any) => (
                        <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="px-3 py-2 text-right font-semibold text-red-700 tabular-nums">{COP(o.pendiente)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {d.cartera_por_cliente.map((c: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    <span className="text-xs text-slate-700 w-5">#{i + 1}</span>
                    <span className="text-sm font-semibold text-slate-700 w-48 truncate">{c.cliente}</span>
                    <div className="flex-1 h-4 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-amber-400 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-sm font-semibold text-amber-700 tabular-nums text-right w-32">{COP(Number(c.total))}</span>
                  </div>
                );
              })}
            </div>
              ...
              <tbody>
                {d.credito_aprobado.map((o: any) => (
                  <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 text-right">{COP(o.valor_total)}</td>
                    <td className="px-3 py-2 text-right font-semibold text-amber-700 tabular-nums">{COP(o.pendiente)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {d && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-card p-4">
              <div className="flex items-center gap-1.5 mb-3">
                <p className="text-[13px] font-semibold text-slate-900">Por Estado</p>
                ...
                {['ABIERTO', 'EN_PROCESO', 'CERRADO'].map(e => {
                  ...
                  return (
                    <div key={e} className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-800 flex-1 capitalize">{e.replace('_', ' ')}</span>
                      <span className="text-sm font-semibold text-slate-900 tabular-nums">{n}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-card p-4">
              <div className="flex items-center gap-1.5 mb-3">
                <p className="text-[13px] font-semibold text-slate-900">Por Tipo</p>
              ...
              <div className="space-y-2">
                {d.por_tipo?.map((r: any) => (
                  <div key={r.tipo} className="flex items-center justify-between">
                    <span className="text-xs text-slate-800">{LABELS_TIPO[r.tipo] || r.tipo}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-slate-900 tabular-nums">{r.total}</span>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-card p-4">
              <div className="flex items-center gap-1.5 mb-3">
                <p className="text-[13px] font-semibold text-slate-900">Top Responsables</p>
              ...
              <div className="space-y-2">
                {d.top_responsables?.map((r: any) => (
                  <div key={r.responsable} className="flex items-center justify-between">
                    <span className="text-xs text-slate-800 truncate">{r.responsable}</span>
                    <span className="text-sm font-semibold text-slate-900 tabular-nums ml-2">{r.total}</span>
                  </div>
                ))}
                {(!d.top_responsables || d.top_responsables.length === 0) && (
                  <p className="text-xs text-slate-700">Sin datos</p>
                )}
              </div>
            </div>
          </div>
          ...
          {d.por_area?.length > 0 && (
            <div className="mb-5">
              <div className="flex items-center gap-2 mb-2">
                <p className="text-[13px] font-semibold text-slate-900">Por Área</p>
              ...
              <div className="flex flex-wrap gap-2">
                {d.por_area.map((r: any) => (
                  <div key={r.area} className="bg-white border border-slate-200 rounded-lg px-3 py-1.5">
                    <span className="text-xs text-slate-800">{r.area}</span>
                    <span className="ml-2 font-semibold text-slate-900">{r.total}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <button onClick={() => setMostrarDetalle(v => !v)}
              className="flex items-center gap-1.5 text-[13px] font-semibold text-slate-900 mb-2 hover:text-slate-700 transition">
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-[13px] font-semibold text-slate-900">Filtrar área:</span>
        {['todas', 'produccion', 'comercial', 'finanzas', 'calidad'].map(a => (
          <button key={a} onClick={() => setAreaFiltro(a)}
            className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition capitalize ${areaFiltro === a ? 'bg-slate-800 text-white' : 'bg-white border border-slate-300 text-slate-800 hover:bg-slate-50'}`}>
            {a === 'todas' ? 'Todas' : a}
          </button>
        ))}
      </div>
      ...
      {alertas.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="font-semibold text-slate-900">Sin alertas para el período seleccionado</p>
          <p className="text-xs text-slate-700">El sistema no detectó deficiencias críticas</p>
        </div>
      ) : (
        <>
          <div className="flex flex-wrap gap-3 mb-4 p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800">
          ...
          {alertas.map((a, i) => (
            <div key={i} className={`border rounded-xl p-4 ${
              ...
              <p className="font-semibold text-slate-900 text-[14px] mb-1">{a.titulo}</p>
              <p className="text-[13px] text-slate-800 mb-2">{a.descripcion}</p>
              <div className="flex items-start gap-1.5 bg-white/60 rounded-lg px-3 py-2">
                ...
                <p className="text-[13px] text-slate-800"><strong className="font-semibold text-slate-900">Acción sugerida:</strong> {a.accion}</p>
              </div>
            </div>
          ))}
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-6 flex items-center gap-3">
        <div className="p-3 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-2xl shadow-card">
          <BarChart2 weight="duotone" className="w-7 h-7 text-white" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Informe Ejecutivo Semanal</h1>
          <p className="text-[13px] text-slate-700 mt-0.5">Visión gerencial completa del sistema</p>
        </div>
      </div>
        ...
        {TABS_CONFIG.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-1.5 px-3 py-2 text-[13px] font-semibold rounded-lg transition ${
                tab === t.id ? 'bg-white text-slate-900 shadow-card border border-slate-200' : 'text-slate-800 hover:text-slate-900 hover:bg-white/60'
              }`}>
