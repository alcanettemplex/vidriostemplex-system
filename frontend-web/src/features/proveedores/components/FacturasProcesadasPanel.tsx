...
import { FileCheck2, Search, RefreshCw, ChevronDown, ChevronUp, Loader2 } from '../../../components/ui/icons';
...
const FacturasProcesadasPanel: React.FC<Props> = ({ proveedores, busquedaInicial }) => {
  ...
  const inputStyle: React.CSSProperties = {
    padding: '7px 12px', borderRadius: RADIUS.md, border: '1px solid var(--border-strong, #c7cdd7)',
    fontSize: FONT.sm, background: 'var(--surface, #fff)', color: 'var(--text, #111620)',
  };
    ...
    >
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <FileCheck2 size={17} style={{ color: 'var(--primary-strong, #4338ca)' }} />
          <span style={{ fontSize: FONT.md, fontWeight: 700 }}>Documentos ya procesados</span>
          <span style={{ fontSize: FONT.sm, color: 'var(--text-muted, #2f3746)', fontWeight: 400 }}>
        ...
        <div style={{ padding: '0 20px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
              <Search size={14} style={{ position: 'absolute', left: 11, top: 9, color: 'var(--text-subtle, #555f71)' }} />
              <input
                value={q}
          ...
          {loading ? (
            <div style={{ padding: '30px 0', textAlign: 'center', color: 'var(--text-muted, #2f3746)', fontSize: FONT.base }}>
              <Loader2 size={20} className="animate-spin" style={{ margin: '0 auto 6px', color: 'var(--primary-strong, #4338ca)', display: 'block' }} />
              Cargando historial…
            </div>
          ) : items.length === 0 ? (
            <div style={{ padding: '26px 0', textAlign: 'center', color: 'var(--text-muted, #2f3746)', fontSize: FONT.base }}>
              {qAplicado || filtroProveedor
                ? 'Ningún documento coincide con el filtro.'
                : 'Todavía no se ha procesado ninguna factura electrónica.'}
            </div>
          ) : (
            <div style={{ border: '1px solid var(--border, #e1e5eb)', borderRadius: RADIUS.lg, overflow: 'auto', maxHeight: 360 }}>
              <table style={{ width: '100%', minWidth: 760, borderCollapse: 'collapse', fontSize: FONT.sm, textAlign: 'left' }}>
                <thead style={{ position: 'sticky', top: 0 }}>
                  <tr style={{ background: 'var(--surface-subtle, #f6f7f9)', borderBottom: '1px solid var(--border, #e1e5eb)' }}>
                    <th style={{ padding: '8px 12px', fontWeight: 600, color: 'var(--text, #111620)' }}>Documento</th>
                    <th style={{ padding: '8px 12px', fontWeight: 600, color: 'var(--text, #111620)' }}>Proveedor</th>
                    <th style={{ padding: '8px 12px', fontWeight: 600, color: 'var(--text, #111620)' }}>Emisión</th>
                    <th style={{ padding: '8px 12px', fontWeight: 600, color: 'var(--text, #111620)' }}>Resultado</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((f) => (
                    <tr key={f.id} style={{ borderBottom: '1px solid var(--border-subtle, #eef0f4)' }}>
                      <td style={{ padding: '8px 12px' }}>
                        <div style={{ fontWeight: 700, color: 'var(--text, #111620)' }}>
                          {f.tipo_documento === 'FACTURA' ? 'FE' : f.tipo_documento === 'NOTA_CREDITO' ? 'NC' : 'ND'}{' '}
                          {f.numero_factura ?? '—'}
                        </div>
                        {f.archivo_origen && (
                          <div style={{ fontSize: FONT.xs, color: 'var(--text-muted, #2f3746)', maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {f.archivo_origen}
                          </div>
                        )}
                      </td>
                      <td style={{ padding: '8px 12px', color: 'var(--text, #111620)' }}>
                      ...
                      <td style={{ padding: '8px 12px' }}>
                        {f.motivo_omision ? (
                          <span style={{ fontSize: FONT.xs, color: '#2f3746', fontWeight: 600 }}>
                            {ETIQUETA_MOTIVO[f.motivo_omision] ?? f.motivo_omision}
                          </span>
                        ) : (
                          <span style={{ fontSize: FONT.xs, color: 'var(--text-muted, #2f3746)' }}>
                            <strong style={{ color: '#047857' }}>{f.lineas_actualizadas}</strong> precio(s) ·{' '}
                            <strong style={{ color: '#b45309' }}>{f.lineas_pendientes}</strong> a mapear ·{' '}
