  ...
  return (
    <div className="grid md:grid-cols-3 gap-6 p-6">
      <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
        <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-900 mb-4 flex items-center gap-2"><FileText className="w-3.5 h-3.5" />Datos de la Orden</h3>
      ...
      <div className="space-y-4">
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-900 mb-4 flex items-center gap-2"><Building2 className="w-3.5 h-3.5" />Cliente</h3>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-900 mb-4 flex items-center gap-2"><User className="w-3.5 h-3.5" />Asesor Responsable</h3>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-slate-900">{odp.asesor?.nombre_completo}</p>
              <p className="text-xs text-slate-700">{odp.asesor?.username} · {odp.asesor?.email}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-900 mb-4">Servicios Incluidos</h3>
          {servicios.length > 0 ? (
          ...
          ) : <p className="text-slate-700 text-sm">Sin servicios adicionales</p>}
        </div>
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-900 mb-4">Ítems de Vidrio ({odp.items?.length || 0})</h3>
          {odp.items?.length > 0 ? (
              ...
              {odp.items.map((item: any, i: number) => (
                <div key={i} className="bg-slate-50 border border-slate-100 rounded-lg p-2.5 text-xs">
                  <div className="flex justify-between items-start">
                    <p className="font-bold text-slate-900">{item.tipo_vidrio || item.item}</p>
                    <Badge className="bg-slate-100 text-slate-800 border-slate-200">{item.cantidad}x</Badge>
                  </div>
                  <p className="text-slate-700 mt-0.5">{item.ancho_mm}mm × {item.alto_mm}mm {item.espesor && `· ${item.espesor}`} {item.color && `· ${item.color}`}</p>
                </div>
              ))}
            </div>
          ) : <p className="text-slate-700 text-sm">Sin ítems registrados</p>}
        </div>
      </div>
    </div>
  );
