            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                ...
                >
                    ...
                    <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                        <h2 className="text-base font-bold text-slate-900">Nueva Orden de Producción</h2>
                        <button onClick={onCancel} className="p-2 rounded-xl text-slate-700 hover:bg-slate-100 transition">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    ...
                    <div className="p-6 space-y-5">
                        <p className="text-sm font-semibold text-slate-900">¿Esta ODP es tuya o de otro asesor?</p>
                            ...
                            <button
                                ...
                                className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition font-semibold text-sm ${
                                    opcion === 'mia'
                                        ...
                                        : 'border-slate-200 text-slate-800 hover:border-blue-300 hover:bg-blue-50/50'
                                }`}
                            >
                                <User className="w-5 h-5" />
                                Mía
                                {currentUser?.nombre_completo && (
                                    <span className="text-[11px] font-normal text-slate-700 text-center leading-tight">
                                        {currentUser.nombre_completo}
                                    </span>
                                )}
                            </button>
                        ...
                        {opcion === 'otro' && (
                            <div>
                                <label className="block text-xs font-semibold text-slate-900 uppercase tracking-wide mb-1.5">
                                    Seleccionar asesor
                                </label>
                                {loading ? (
