...
import { Package, Plus, DollarSign } from '../../../components/ui/icons';
...
const estadoCotColor: Record<string, string> = {
  ...
  vencida: 'bg-slate-100 text-slate-800 border-slate-200',
};
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span className="font-bold text-blue-700 text-lg">{cot.numero_cot}</span>
                    <Badge className={estadoCotColor[cot.estado] || 'bg-slate-100 text-slate-800 border-slate-200'}>{cot.estado}</Badge>
                  </div>
                  <p className="text-xs text-slate-700">{cot.asesor?.nombre_completo} · {new Date(cot.fecha_creacion).toLocaleDateString('es-CO')} · Válida {cot.validez_dias} días</p>
                  {cot.descuento > 0 && (
                ...
                <div className="text-right">
                  <p className="text-2xl font-extrabold text-slate-900">{fmt(cot.valor_total)}</p>
                  <p className="text-xs text-slate-700 mt-0.5">TOTAL NETO</p>
                  <p className="text-xs font-bold text-slate-900 mt-0.5">{cot.forma_pago}</p>
                </div>
              </div>
              {cot.notas && <p className="text-xs text-slate-700 italic mt-2 pt-2 border-t border-slate-100">"{cot.notas}"</p>}
            </div>
          ))}
      </div>
    </div>
