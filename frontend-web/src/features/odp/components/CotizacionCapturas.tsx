...
import { Camera, Plus, Trash2, Pencil, X, ZoomIn, Check, Hash } from '../../../components/ui/icons';
    ...
    <div>
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] font-semibold text-blue-600 uppercase tracking-wider mb-1">N° Cotización</p>
          {editandoNumCot ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setEditandoNumCot(false); setNumCot(numeroCotizacion || ''); }}
                className="p-1.5 border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 transition">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <span className={`text-sm font-bold ${numCot ? 'text-blue-800' : 'text-slate-700 italic'}`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold uppercase tracking-widest text-slate-900 flex items-center gap-2">
          ...
          {capturas.length > 0 && (
            <span className="ml-1 px-2 py-0.5 text-[11px] font-semibold bg-indigo-100 text-indigo-700 rounded-full">
              {capturas.length}
            </span>
          )}
        </h3>
      ...
      {showUpload && (
        <div className="mb-4 p-4 bg-indigo-50 border border-indigo-200 rounded-2xl space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wider">Nueva captura</p>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 py-2 text-sm font-bold border border-slate-200 text-slate-900 rounded-xl hover:bg-slate-50 transition"
            >
              Cancelar
            </button>
            <button
      ...
      {/* Galería */}
      {loading ? (
        <div className="text-center py-6 text-slate-700 text-sm">Cargando...</div>
        ...
        <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center text-slate-700">
          ...
          <p className="font-medium text-sm">No hay capturas registradas</p>
        </div>
      ) : (
            ...
            <div key={c.id} className="group relative bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition">
              ...
              <div className="p-2">
                {editandoId === c.id ? (
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => setEditandoId(null)}
                        className="px-2 py-1 text-[11px] font-bold border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 transition">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    {c.nota && <p className="text-[11px] text-slate-800 italic mb-1 line-clamp-2">"{c.nota}"</p>}
                    ...
                    {puedeEditar(c) && (
                      <div className="flex gap-1 mt-1.5">
                        ...
                        <button onClick={() => handleDelete(c)}
                          className="py-1 px-2 text-[11px] font-bold border border-red-200 text-red-700 rounded-lg hover:bg-red-50 transition flex items-center justify-center">
