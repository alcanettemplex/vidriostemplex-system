...
const getLetra = (idx: number): string => {
    ...
    const primerCiclo = Math.floor(idx / 26) - 1;
    const segundoCiclo = idx % 26;
    return abc[primerCiclo] + abc[segundoCiclo];
};
...
const PrintableTalonario: React.FC<PrintableTalonarioProps> = ({ odp }) => {
    ...
    const numerosExterno = odp.pedidos_pv?.length > 0
        ? odp.pedidos_pv.map((pv: any) => pv.numero_pedido).join(', ')
        : odp.numero_pedido_proveedor || null;
        ...
        <>
            <style>
                ...
                @media print {
                    ...
                    .talonario-page { page-break-after: always; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
                    .talonario-page:last-child { page-break-after: avoid; }
            ...
            {Array.from({ length: paginas }, (_, pageIdx) => {
                ...
                return (
                    <div
                        key={pageIdx}
                        className="talonario-page print-root block w-[21.5cm] min-h-[29cm] print:min-h-0 bg-white shadow-xl print:shadow-none text-black font-sans text-[10px] mx-auto overflow-hidden print:overflow-visible mb-6 print:mb-0"
                    >
                        <div className="print-container p-2 print:p-0">
                            ...
                            <table className="excel-table text-center uppercase">
                                ...
                                <tbody>
                                    {Array.from({ length: pageIdx === 0 ? ITEMS_PAG1 : ITEMS_CONT }, (_, rowIdx) => {
                                        ...
                                        return (
                                            <tr key={absoluteIdx} className="h-[22px]">
                                                <td className="font-bold">{letra}</td>
                                    ...
                                    {/* Totales y asesor solo en última página */}
                                    {esUltimaPagina && (
                                        <>
                                            <tr>
                            ...
                            {/* Nota PUL/ACABADOS - todas las páginas */}
