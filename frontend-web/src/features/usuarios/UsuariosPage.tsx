...
import { Users, Plus, Pencil, X, Eye, EyeOff, UserX, UserCheck } from '../../components/ui/icons';
...
const ROLES = [
  ...
  { value: 'marketing', label: 'Marketing (Solo Lectura CRM)', color: 'bg-pink-100 text-pink-700' }
];
    ...
    <div className="p-6 w-full space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-4">
        <div>
          ...
          <p className="text-slate-700 mt-1">Administra los roles y accesos al sistema</p>
        </div>
        <button onClick={openCreate}
          className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white font-semibold rounded-xl shadow-md shadow-indigo-200 hover:bg-indigo-700 transition">
        ...
        {ROLES.map(rol => {
          ...
          return (
            <div key={rol.value} className={`p-4 rounded-xl border flex justify-between items-center ${rol.color} border-current/20`}>
              <p className="text-xs font-semibold truncate">{rol.label}</p>
          ...
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              {['Usuario', 'Nombre Completo', 'Email', 'Rol', 'Gestión PV', 'Estado', 'Acciones'].map(h => (
                <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-slate-900 uppercase tracking-wider">{h}</th>
              ))}
            </tr>
          </thead>
            ...
            {loading ? Array.from({length: 5}).map((_,i)=>(
              ...
              return (
                <motion.tr key={u.id} initial={{opacity:0}} animate={{opacity:1}} className={`hover:bg-slate-50 transition-colors ${inactivo ? 'opacity-50' : ''}`}>
                  <td className="px-5 py-4 font-mono text-sm font-semibold text-slate-900">{u.username}</td>
                  <td className="px-5 py-4 font-semibold text-slate-900">{u.nombre_completo}</td>
                  <td className="px-5 py-4 text-slate-800">{u.email || '—'}</td>
                  <td className="px-5 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border border-current/20 ${cfg.color}`}>{cfg.label}</span>
                  </td>
                  <td className="px-5 py-4">
                    {u.puede_gestionar_pv
                      ? <span className="px-2 py-1 rounded-full text-xs font-semibold bg-orange-100 text-orange-700 border border-orange-200">Sí</span>
                      : <span className="text-slate-400 text-xs">—</span>
                    }
                  </td>
                  <td className="px-5 py-4">
                    {inactivo
                      ? <span className="px-2 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200">Inactivo</span>
                      : <span className="px-2 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700 border border-emerald-200">Activo</span>
                    }
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(u)} className="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition" title="Editar">
                        <Pencil className="w-4 h-4"/>
                      </button>
                      {u.rol !== 'admin' && (
                        inactivo
                          ? <button onClick={() => handleReactivar(u)} className="p-1.5 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Reactivar">
                              <UserCheck className="w-4 h-4"/>
                            </button>
                          : <button onClick={() => handleDesactivar(u.id, u.nombre_completo)} className="p-1.5 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition" title="Desactivar">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}}
            ...
            <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100">
              <h2 className="text-xl font-bold text-slate-900">{editUser ? 'Editar Usuario' : 'Nuevo Usuario'}</h2>
              <button onClick={()=>setShowForm(false)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 transition"><X className="w-5 h-5"/></button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-900 mb-1 uppercase tracking-wide">Nombre Completo *</label>
                <input value={form.nombre_completo} onChange={e=>setForm(p=>({...p,nombre_completo:e.target.value}))} required
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-900 mb-1 uppercase tracking-wide">Username *</label>
                  <input value={form.username} onChange={e=>setForm(p=>({...p,username:e.target.value}))} required
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-900 mb-1 uppercase tracking-wide">Contraseña {editUser ? '(deja vacío p/no cambiar)' : '*'}</label>
                  <div className="relative">
                    <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={e=>setForm(p=>({...p,password:e.target.value}))} required={!editUser}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 bg-white pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                    <button type="button" onClick={()=>setShowPassword(p=>!p)} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-900 mb-1 uppercase tracking-wide">Email</label>
                <input type="email" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-900 mb-1 uppercase tracking-wide">Rol *</label>
              ...
              <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <div>
                    <p className="text-sm font-semibold text-slate-900">Puede gestionar Pedidos PV</p>
                    <p className="text-xs text-slate-700">Acceso a la pestaña "Por Gestionar" en el módulo Pedidos PV</p>
                  </div>
                </label>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={()=>setShowForm(false)} className="flex-1 py-2.5 font-semibold text-slate-700 border border-slate-300 rounded-xl hover:bg-slate-50 transition">Cancelar</button>
                <button type="submit" className="flex-1 py-2.5 font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition shadow-md shadow-indigo-200">
