...
import { PhoneMissed, RefreshCw, RotateCcw, MessageSquare, User, Phone, Tag, Calendar } from '../../../components/ui/icons';
import {
...
const FUENTE_COLOR: Record<string, string> = {
  ...
  Presencial:      'bg-slate-100 text-slate-800',
...
const SinRespuestaTab: React.FC<SinRespuestaTabProps> = ({ fecha_desde, fecha_hasta, busqueda = '', asesor_id }) => {
  ...
  if (loadingSinRespuesta) {
    return (
      <div className="flex items-center justify-center py-20 text-slate-700 text-sm gap-2">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col md:flex-row md:items-center gap-4">
        <div className="flex items-center gap-3 flex-1">
          <div className="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center">
            <PhoneMissed className="w-5 h-5 text-amber-700" />
          </div>
          <div>
            <p className="font-bold text-slate-900 text-sm">Leads sin respuesta — {fecha_desde || '—'} → {fecha_hasta || '—'}</p>
            <p className="text-xs text-slate-700 mt-0.5">
              ...
              {busqueda && leadsFiltrados.length !== leadsSinRespuesta.length && (
                <span className="ml-1 text-indigo-700 font-bold">(filtrado de {leadsSinRespuesta.length})</span>
              )}
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(porFuente).map(([fuente, count]) => (
            <span
              key={fuente}
              className={`text-[11px] font-semibold px-2.5 py-1 rounded-full border ${FUENTE_COLOR[fuente] || 'bg-slate-100 text-slate-800'}`}
            >
              {fuente}: {count}
            </span>
          ))}
        ...
        <button
          onClick={cargar}
          className="p-2 rounded-lg hover:bg-amber-100 text-amber-700 transition-colors self-start md:self-auto"
          title="Recargar"
        >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-900 uppercase tracking-widest">Contacto</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-900 uppercase tracking-widest">Fuente</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-900 uppercase tracking-widest">Producto</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-900 uppercase tracking-widest">Mensaje de entrada</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-900 uppercase tracking-widest">Fecha</th>
                  ...
                  <tr key={lead.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <p className="font-bold text-slate-900 leading-tight">{lead.nombre}</p>
                    ...
                    {/* Fuente */}
                    <td className="px-4 py-3">
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${FUENTE_COLOR[lead.fuente_lead] || 'bg-slate-100 text-slate-800'}`}>
                        {lead.fuente_lead || '—'}
                      </span>
                    </td>
                    {/* Producto */}
                    ...
                    <td className="px-4 py-3 max-w-xs">
                      {lead.mensaje_entrada ? (
                        <div className="flex items-start gap-1.5">
                          ...
                          <p className="text-xs text-slate-800 italic line-clamp-2">{lead.mensaje_entrada}</p>
                        </div>
                      ) : (
                        <span className="text-xs text-slate-500 italic">Sin mensaje</span>
                      )}
                    </td>
                    {/* Fecha */}
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold rounded-lg bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-100 transition-colors whitespace-nowrap"
