...
import { Camera, Plus, Trash2, Pencil, X, ZoomIn, Check, ImageOff } from '../../../components/ui/icons';
import {
  apiGetLeadImagenes,
  apiUploadLeadImagen,
    ...
    <div className="bg-white border border-slate-100 rounded-2xl p-5 space-y-4 shadow-sm">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-semibold text-slate-900 uppercase tracking-wider flex items-center gap-2">
          ...
          Imágenes del Lead
          {imagenes.length > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-semibold bg-indigo-100 text-indigo-700 rounded-full">
      ...
      {showUpload && (
        <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-xl space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wider">Nueva imagen</p>
            <button onClick={cancelarUpload}>
              <X className="w-4 h-4 text-slate-500 hover:text-slate-600" />
            </button>
          </div>
          ...
          >
            {preview ? (
              ...
              <div className="text-indigo-700">
                ...
                <p className="text-[11px] text-slate-700 mt-0.5">JPG, PNG, WEBP</p>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <button onClick={cancelarUpload}
              className="flex-1 py-2 text-xs font-bold border border-slate-200 text-slate-900 rounded-xl hover:bg-slate-50 transition">
              Cancelar
            </button>
      ...
      {/* Galería */}
      {loading ? (
        <div className="text-center py-4 text-slate-700 text-xs">Cargando...</div>
      ) : imagenes.length === 0 && !showUpload ? (
        <div className="border-2 border-dashed border-slate-100 rounded-xl p-6 text-center text-slate-700">
          <ImageOff className="w-8 h-8 mx-auto mb-2 text-slate-500" />
          <p className="text-xs font-bold">Sin imágenes adjuntas</p>
          {puedeSubir && (
            <p className="text-[11px] text-slate-700 mt-0.5">Usa el botón "Subir imagen" para agregar</p>
          )}
        </div>
      ) : (
                ...
                {editandoId === img.id ? (
                  <div className="space-y-1">
                    <textarea value={editNota} onChange={e => setEditNota(e.target.value)} rows={2}
                      className="w-full border border-slate-200 rounded-lg px-2 py-1 text-[11px] focus:outline-none focus:ring-1 focus:ring-indigo-400 resize-none" />
                    <div className="flex gap-1">
                      <button onClick={() => handleGuardarNota(img)} disabled={guardandoNota}
                        className="flex-1 py-1 text-[11px] font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition disabled:opacity-40 flex items-center justify-center gap-1">
                        <Check className="w-3 h-3" /> Guardar
                      </button>
                      <button onClick={() => setEditandoId(null)}
                        className="px-2 py-1 text-[11px] font-semibold border border-slate-200 text-slate-900 rounded-lg hover:bg-slate-50 transition">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    {img.nota && (
                      <p className="text-[11px] text-slate-800 italic mb-1 line-clamp-2">"{img.nota}"</p>
                    )}
                    ...
                    {puedeEditar(img) && (
                      <div className="flex gap-1 mt-1.5">
                        <button onClick={() => { setEditandoId(img.id); setEditNota(img.nota || ''); }}
                          className="flex-1 py-1 text-[11px] font-bold border border-slate-200 text-slate-900 rounded-lg hover:bg-slate-50 transition flex items-center justify-center gap-1">
                          <Pencil className="w-2.5 h-2.5" /> Nota
                        </button>
                        <button onClick={() => handleEliminar(img)}
                          className="py-1 px-2 text-[11px] font-semibold border border-red-200 text-red-700 rounded-lg hover:bg-red-50 transition flex items-center justify-center">
