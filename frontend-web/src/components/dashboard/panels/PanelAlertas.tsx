...
export const PanelAlertas: React.FC<{
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white p-8 rounded-2xl border border-emerald-200 shadow-card flex flex-col items-center justify-center text-center">
        <ShieldCheck weight="duotone" className="w-10 h-10 text-emerald-600 mb-3" />
        <h2 className="text-[15px] font-semibold text-slate-900 mb-1">Sin alertas activas</h2>
        ...
        <span className="text-[11px] uppercase font-semibold text-slate-700 mt-4 tracking-wide">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-card px-5 py-4 flex flex-wrap items-center gap-x-6 gap-y-2">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-rose-50 ring-1 ring-rose-100">
            <Bell weight="duotone" className="w-5 h-5 text-rose-600" />
          </div>
          <div>
            <p className="text-[22px] font-extrabold text-slate-900 leading-none tabular-nums">{resumen.total}</p>
            <p className="text-[11px] uppercase font-semibold text-slate-900 tracking-wide mt-1">Alertas</p>
          </div>
        </div>

        <div className="h-8 w-px bg-slate-100 hidden sm:block" />

        <Metrica valor={resumen.vencidas} etiqueta="Vencidas" color="text-rose-700" />
        <Metrica valor={resumen.produccion.length} etiqueta="Producción" color="text-slate-900" />
        <Metrica valor={resumen.cartera.length} etiqueta="En mora" color="text-slate-900" />
        <Metrica valor={montoCorto(resumen.montoRiesgo)} etiqueta="En riesgo" color="text-amber-700" />

        <span className="ml-auto text-[12px] text-slate-700 flex items-center gap-1.5">
      ...
      {grupos.map(grupo => {
        ...
        return (
          <div key={grupo.clave} className="bg-white rounded-2xl border border-slate-200 shadow-card overflow-hidden">
            <button
              onClick={() => setColapsado(p => ({ ...p, [grupo.clave]: abierto }))}
              className="w-full px-4 py-3 flex items-center gap-2.5 border-b border-slate-100 hover:bg-slate-50 transition-colors text-left"
            >
              <ChevronDown
                className={`w-4 h-4 text-slate-600 transition-transform ${abierto ? '' : '-rotate-90'}`}
              />
              <Icono weight="duotone" className="w-[18px] h-[18px] text-slate-700" />
              <span className="text-[14px] font-semibold text-slate-900">
                {grupo.titulo}
              </span>
              <span className="text-[12px] text-slate-700">{grupo.subtitulo}</span>
            </button>
...
const Metrica: React.FC<{ valor: number | string; etiqueta: string; color: string }> = ({
  ...
  <div>
    <p className={`text-[22px] font-extrabold leading-none tabular-nums whitespace-nowrap ${color}`}>{valor}</p>
    <p className="text-[11px] uppercase font-semibold text-slate-900 tracking-wide mt-1">{etiqueta}</p>
  </div>
);
...
const FilaAlerta: React.FC<{
  ...
  return (
    ...
    >
      ...
      <span className="text-[12px] font-semibold text-slate-900 shrink-0 w-[104px] truncate">
      ...
      {esCartera && alerta.monto != null && (
        <span className="text-[12px] font-semibold text-slate-900 tabular-nums shrink-0 hidden sm:block">
          {moneda(alerta.monto)}
        </span>
      )}
      ...
      <button
        ...
        className="shrink-0 flex items-center gap-1 px-2 py-1 text-[12px] font-semibold text-slate-800
                   border border-slate-200 rounded-md opacity-0 group-hover:opacity-100 focus:opacity-100
